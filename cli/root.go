// Package cli assembles the itunes command tree from the itunes
...
// NewApp assembles the kit application from the itunes domain. The
...
//
// To add a command, declare it in itunes/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := itunes.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(itunes.Domain{}).Register(app)
