...
describes what users get. Pick a format with `-o`, or let itunes choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
itunes <command> -o table   # aligned columns for reading
itunes <command> -o jsonl   # one JSON object per line, for piping
itunes <command> -o json    # a single JSON array
itunes <command> -o csv     # spreadsheet friendly
itunes <command> -o tsv     # tab-separated
itunes <command> -o url     # just the URL column
itunes <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
itunes <command> --fields id,title,url
```
...
```bash
itunes <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
itunes <command>            # a table, because this is a terminal
itunes <command> | wc -l    # JSONL, because this is a pipe
```
